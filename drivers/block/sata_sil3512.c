 ...
 * MA 02111-1307 USA
 *
 * This is a driver for Silicon Image sil3512 sata chip modelled on
 * the ata_piix driver
 */
/*
 * Based on sata_sil3114.c
 */
...
#include <ide.h>
#include <libata.h>
#include "sata_sil3512.h"

/* Convert sectorsize to wordsize */
#define ATA_SECTOR_WORDS (ATA_SECT_SIZE/2)

#if defined(CONFIG_SYS_SATA_DEVS_PER_BUS)
#if CONFIG_SYS_SATA_DEVS_PER_BUS > 2
#error "CONFIG_SYS_SATA_DEVS_PER_BUS must be <= 2"
#endif	/* CONFIG_SYS_SATA_DEVS_PER_BUS > 2 */
#else	/* CONFIG_SYS_SATA_DEVS_PER_BUS */
#define CONFIG_SYS_SATA_DEVS_PER_BUS	2
#endif	/* CONFIG_SYS_SATA_DEVS_PER_BUS */

#if ! defined(CONFIG_SYS_SATA_MAXBUS)
#define CONFIG_SYS_SATA_MAXBUS	1
#endif	/* CONFIG_SYS_SATA_MAXBUS */

#if ! defined(CONFIG_SYS_SATA_MAX_DEVICE)
#define CONFIG_SYS_SATA_MAX_DEVICE	(CONFIG_SYS_SATA_MAXBUS * CONFIG_SYS_SATA_DEVS_PER_BUS)
#endif	/* CONFIG_SYS_SATA_MAX_DEVICE */

#define pci_to_mem(dev,addr,mask)	((addr & PCI_REGION_IO) ? pci_io_to_phys(dev, addr & mask) : pci_mem_to_phys(dev, addr & mask))

/* Forwards */
u8 sil3512_spin_up(int num);
u8 sil3512_spin_down(int num);
...
static baseaddr_t baseaddr[CONFIG_SYS_SATA_MAXBUS];
extern block_dev_desc_t sata_dev_desc[CONFIG_SYS_SATA_MAX_DEVICE];

static struct sata_port port[CONFIG_SYS_SATA_MAX_DEVICE];

static void
output_data(struct sata_ioports *ioaddr, u16 *sect_buf, int words)
{
	while (words --) {
...
static int
input_data(struct sata_ioports *ioaddr, u16 *sect_buf, int words)
{
	while (words --) {
...
sata_bus_softreset(int num)
{
	...
	if (status & ATA_BUSY) {
		printf("sata%u is slow to respond,plz be patient\n", num);
	}
	...
	if (status & ATA_BUSY) {
		printf("sata%u failed to respond : ", num);
...
u8
sil3512_spin_down(int num)
{
	u8 status = 0;

	debug("Spin down disk #%d\n", num);

	if (!(port[num].dev_mask & 0x01)) {
		debug("Device sata%d is not present\n", num);
		return 1;
	}
	...
	if (status == 0x01) {
		printf("Failed to check power mode on sata%d\n", num);
		return 1;
	}

	if (!((status = sata_chk_status(&port[num].ioaddr, 0)) & ATA_DRDY)) {
		printf("Device sata%d not ready\n", num);
		return 1;
	}
...
u8
sil3512_spin_up(int num)
{
	u8 status = 0;

	debug("Spin up disk #%d\n", num);
...
static u8
wait_for_irq(int num, unsigned int max)
{
	int unit;
	int ch;
	baseaddr_t *bar;
	...
	unit = num / CONFIG_SYS_SATA_DEVS_PER_BUS;
	ch = num % CONFIG_SYS_SATA_DEVS_PER_BUS;
	bar = &baseaddr[unit];
	mem = bar->bar[5];
	...
	do {
		if (readl(mem) & VND_TF_CNST_INTST) {
			break;
		}
		udelay(1000);
		max--;
...
static u8
sata_busy_wait(struct sata_ioports *ioaddr, int bits,
	       unsigned int max, u8 usealtstatus)
{
	u8 status;
...
static u8
sata_chk_status(struct sata_ioports *ioaddr, u8 usealtstatus)
{
	if (!usealtstatus) {
...
*/
static u8
do_one_read(int device, ulong block, u8 blkcnt, u16 * buff, uchar lba48)
{
	...
	while (blkcnt--) {
		...
		if (wait_for_irq(device, 500)) {
			printf("sata%u irq failed\n", device);
			return sr;
		}

		status = sata_chk_status(&port[device].ioaddr, 0);
		if (status & ATA_ERR) {
			printf("sata%u error %d\n", device,
...
ulong
sata_read(int device, ulong block, lbaint_t blkcnt, void *buff)
{
	ulong n = 0, sread;
	u16 *buffer = (u16 *) buff;
	u8 status = 0;
...
ulong
sata_write(int device, ulong block, lbaint_t blkcnt, const void *buff)
{
	ulong n = 0;
	u16 *buffer = (u16 *) buff;
	...
	while (blkcnt-- > 0) {
		...
		if (status & ATA_BUSY) {
			printf("sata%u failed to respond\n", port[num].port_no);
			return n;
		}
#ifdef CONFIG_LBA48
		if (lba48) {
...
init_sata(int dev)
{
	...
	for (unit = 0; unit < CONFIG_SYS_SATA_MAXBUS; unit ++) {
		bar = &baseaddr[unit];
		devno = pci_find_device(SIL_VEND_ID, SIL3512_DEVICE_ID, unit);
		if (devno == -1) {
			res ++;
			debug("pci_find_device(SIL_VEND_ID, SIL3512_DEVICE_ID, %d) failed\n", unit);
			goto done;
		}
		...
		if ((bar->bar[0] == 0xFFFFFFFF) ||
		    ...
		    (bar->bar[2] == 0xFFFFFFFF) ||
		    (bar->bar[3] == 0xFFFFFFFF) ||
		    (bar->bar[4] == 0xFFFFFFFF) ||
		    (bar->bar[5] == 0xFFFFFFFF)) {
		...
		debug("baseaddr[3] = 0x%08x\n", bar->bar[3]);
		debug("baseaddr[4] = 0x%08x\n", bar->bar[4]);
		debug("baseaddr[5] = 0x%08x\n", bar->bar[5]);
		...
		if (cls) {
			...
			writel(cls << 8 | cls, bar->bar[5] + VND_FIFOCFG_CH0);
			writel(cls << 8 | cls, bar->bar[5] + VND_FIFOCFG_CH1);
		}
		else {
...
sata_bus_probe(int portno)
{
	...
	unit = portno / CONFIG_SYS_SATA_DEVS_PER_BUS;
	ch = portno % CONFIG_SYS_SATA_DEVS_PER_BUS;
	bar = &baseaddr[unit];
	mem = bar->bar[5];
...
sata_phy_reset(int portno)
{
	...
	switch (ch) {
	case 0:
		mem += VND_SCONTROL_CH0;
		break;
	case 1:
		mem += VND_SCONTROL_CH1;
		break;
	default:
		return 0;
	}
	val = readl(mem);
	writel(val | SATA_SC_DET_RST, mem);
	msleep(150);
	writel(val & ~SATA_SC_DET_RST, mem);
	return 0;
}
...
scan_sata(int dev)
{
	...
	unit = dev / CONFIG_SYS_SATA_DEVS_PER_BUS;
	ch = dev % CONFIG_SYS_SATA_DEVS_PER_BUS;
	bar = &baseaddr[unit];

	/* A bit brain dead, but the code has a legacy */
	switch (ch) {
	case 0:
		port[dev].port_no = dev;
		port[dev].ioaddr.cmd_addr = bar->bar[5] + VND_TF0_CH0;
		port[dev].ioaddr.altstatus_addr = port[dev].ioaddr.ctl_addr =
			(bar->bar[5] + VND_TF2_CH0) | ATA_PCI_CTL_OFS;
		port[dev].ioaddr.bmdma_addr = bar->bar[5] + VND_BMDMA_CH0;
		break;
	case 1:
		port[dev].port_no = dev;
		port[dev].ioaddr.cmd_addr = bar->bar[5] + VND_TF0_CH1;
		port[dev].ioaddr.altstatus_addr = port[dev].ioaddr.ctl_addr =
			(bar->bar[5] + VND_TF2_CH1) | ATA_PCI_CTL_OFS;
		port[dev].ioaddr.bmdma_addr = bar->bar[5] + VND_BMDMA_CH1;
		break;
	default:
		printf("Tried to scan unknown port: sata%d\n", dev);
		return 1;
	}
