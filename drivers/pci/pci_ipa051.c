...
static inline void nxp_ipa051_pci_clear_status( void )
{
	...
	pci_stat = inl(IPA051_GPPM_INT_STATUS);
	outl(pci_stat, IPA051_GPPM_INT_CLR);
}
			       ...
			       unsigned int	*val )
{
	...
	if ((pci_cmd == IPA051_GPPM_CTRL__GPPM_CMD_IO_RD) ||
		...
		printf("%s timeout (GPPM_CTRL=%X) ioaddr %X pci_cmd %X\n",
		       __FUNCTION__, inl(IPA051_GPPM_CTRL), ioaddr,
		       pci_cmd);
	}
...
void init_nxp_ipa051_pci( struct pci_controller	*hose )
{
	...
	pci_set_ops (hose,
		    nxp_ipa051_pcibios_read_config_byte,
		    nxp_ipa051_pcibios_read_config_word,
		    nxp_ipa051_pcibios_read_config_dword,
		    nxp_ipa051_pcibios_write_config_byte,
		    nxp_ipa051_pcibios_write_config_word,
		    nxp_ipa051_pcibios_write_config_dword);
